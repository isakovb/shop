...
class Builder extends HookableBuilder
{
    ...
    */
    ...
    protected function searchSelect(SearchableSubquery $subquery, ColumnCollection $columns, array $words)
    {
        ...
        $wholeWord = implode(' ', $words);
        $wholeWord = preg_replace('/[\*]+/', '', $wholeWord);
        $extraCase = "case when %s = '{$wholeWord}' then 30 else 0 end + ";
        $extraCase = sprintf($extraCase, $column->getWrapped());
        $cases[0] = $extraCase.$cases[0];
